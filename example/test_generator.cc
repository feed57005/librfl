...
int Gen::EndPackage(Package const *pkg) {
  ...
  for (Enum *e : enums_) {
    out_ << "  {\n"
         ...
         << "    " << e->name() << "_enum->enum_name_ = \"" << GetEnumRegisterName(e) << "\";\n";
...
int Gen::EndClass(Class const *clazz) {
  ...
  for (size_t i = 0; i < clazz->GetNumFields(); i++) {
    ...
    if (kind.compare("number") == 0) {
      ...
      out_ << "  instance->AddProperty(new test::NumericProperty<" << type << ">(\"" << id
           << "\", \"" << name << "\", " << field->offset() << ", class_id(), "
           << "rfl::AnyVar((" << type << ")(" << default_value << ")), "
           << min << ", "
           << max << ", "
           << step << ", "
           << page_step << ", "
      ...
      if (field->type_ref().kind() == TypeRef::kEnum_Kind) {
        ...
        out_ << "  instance->AddProperty(new test::EnumProperty(enum_" << id
             ...
             << ", " << enm->name() << "::" << anno.GetEntry("default")
             << "));\n";
      } else {
        char const *type = field->type_ref().type_name();
        char const *default_value = anno.GetEntry("default");
        out_ << "  instance->AddProperty(new test::Property(\"" << id
             << "\", \"" << name << "\", " << field->offset()
             << ", class_id(), "
             << "rfl::AnyVar((" << type << ") " << type << "::" << default_value
             << ")));\n";
      }
    } else {
      char const *default_value = anno.GetEntry("default");
