...
char const * const kHeaderSuffix = ".rfl.h";
char const * const kSrcSuffix = ".rfl.cc";
char const *kFilePrologue = "// Generated by rfl-scan, do not modify\n\n";

// static
std::string Gen::FilenameForPackageFile(PackageFile const *file, bool full, bool header) {
...
// static
bool Gen::WriteStreamToFile(std::string const &file,
...
// static
std::string Gen::HeaderGuard(std::string const &name, bool begin) {
...
int Gen::BeginFile(PackageFile const *file) {
  if (!file->is_dependency() && file->GetNumClasses()) {
    std::cout << "begin file: " << file->source_path() << std::endl;
    hout_.str(std::string());
    out_.str(std::string());
    src_includes_.clear();
    h_includes_.clear();
...
int Gen::EndFile(PackageFile const *file) {
  ...
  std::cout << "end file: " << h_file << std::endl;
